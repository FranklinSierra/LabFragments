package com.example.franklinsierra.labfragments.Util;
...
public class Util {
    ...
    //  ++++    metodo que contiene todos los colores a trabajar    ++++    //
    ...
    //  ++++    metodo que contine los correos ficticios    ++++    //

    public static List<Mail> correos() {
    ...
    //  ++++    metodo que elige aleatoriamente los colores del primer metodo   ++++    //

    public static String getRandomColors() {
        //se saca un numero aleatorio entre 0 y 14 (cantidad de colores a trabajar)
        int randomNumber = new Random().nextInt(colors.length) + 0;   // Se le pasa el #max (color.length) y el #min (el cero)
        //se retorna el color
